...
function setPermission(){
    ...
    if (document.cookie) {
        ...
        cookiesArray.forEach(data => {
            data = data.split('=');
            data[0] = data[0].trim(' ');
            if(data[0] === Config.config.tokenName){
            token = data[1];
            }
        });
        if(token !== ""){
    ...
    if(decoded[Config.config.tokenAttributeName]){
        terria.setPermission(decoded[Config.config.tokenAttributeName]);
    }
}
...
//tokenの検証を行う
fetch(Config.config.apiUrl + "/auth/checkAuth")
.then(res => {
    if(res.status === 200){
        setPermission();
    }else{
