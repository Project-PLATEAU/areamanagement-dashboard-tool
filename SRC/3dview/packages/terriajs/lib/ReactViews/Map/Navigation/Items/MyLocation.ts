...
class MyLocation extends MapNavigationItemController {
  ...
  zoomToMyLocation(position: Position) {
    ...
    if (this.augmentedVirtualityEnabled()) {
      ...
      this.terria.focusMapPlaceAndAttributeDisplay(longitude,latitude,null,null,this.viewState,200);
    ...
    runInAction(() => {
      ...
      this._marker.setTrait(CommonStrata.user, "idealZoom",{targetLongitude:longitude,targetLatitude:latitude,targetHeight:100,heading:0 ,pitch:30 ,range:500});
