  ...
  createReactClass({
    ...
    toggleVisibility() {
      runInAction(() => {
        ...
        if (this.props.item.show && checkIfSwitchableStyles(this.props.item)) {
          try{
            //疑似的なstyleを仮で当て込み
            //render時に変更を検知させる
            const customStyle={};
            ...
            const condition = [];
            condition.push("true");
            condition.push("color('#FFFFFF',1)");
            customStyle["color"]["conditions"].push(condition);
            this.props.item.setTrait(CommonStrata.user, "style", customStyle);
