...
export const FeatureInfoSection = observer(
  createReactClass({
    ...
    componentDidMount() {
      if(this.props.feature.data?.properties?.activity_id > 0){
        this.setState({
          currentActivityId: this.props.feature.data.properties.activity_id,
          parentActivityId: this.props.feature.data.properties.parent_activity_id
        });
        this.fetchActivityHistory(this.props.feature.data.properties.parent_activity_id,this.props.feature.data.properties.activity_type,false);
        this.fetchActivityData(this.props.feature.data.properties.activity_id);
      }
    },
    ...
    render() {
      ...
      if(currentActivityId > 0){
        if(this.props.feature.data.properties.activity_type === 1){
          activityUrl = Config.config.activityDetailsUrlForErimane;
        }else if(this.props.feature.data.properties.activity_type === 2){
          activityUrl = Config.config.activityDetailsUrlForEvent;
        }
      }
      const activityUrlForParameterName = Config.queryParameter.parameterNameForActivityDetailsUrl;
          ...
          <If condition={this.props.isOpen}>
            <section className={Styles.content}>
                  ...
                  <div style={{ maxWidth: "350px" }}>
                    ...
                    {this.props.permission === "admin" && (
                      <div style={{ display:"flex" }}>
                        <button
                          style={{ margin: "0 auto", lineHeight: 30 + "px", width: 120 + "px", marginTop: 15 + "px", marginBottom: 15 + "px" }}
                        ...
                        onClick={evt => {
                          evt.preventDefault();
                          evt.stopPropagation();
                          window.open(activityUrl + "?"+activityUrlForParameterName+"=" + parentActivityId,'_metabase');
                          return false;
                        }}
                      >
                        <span>詳細</span>
              ...
              <div id="featureInfoControll" style={{ position: "absolute", left: -99999 + "px" }} onClick={evt => {
                evt.preventDefault();
                evt.stopPropagation();
                if(currentActivityId > 0 && this.props.viewState.parentActivityId > 0){
                  this.fetchActivityHistory(this.props.viewState.parentActivityId,this.props.feature.data.properties.activity_type,true);
                }
              }}>
              </div>
            </section>
