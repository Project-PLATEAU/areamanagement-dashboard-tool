...
export default class ViewState {
  ...
  // ログイン画面
  @observable showLoginMenu: boolean = false;
  ...
  // 町丁目検索画面
  @observable showSearchMenu: boolean = false;

  // 活動登録
  @observable showActivityMenu: boolean = false;
  @observable parentActivityId:number = 0;
  @observable currentActivityId:number = 0;
  @observable activityType:number = 1;

  // 投稿レイヤ
  @observable showPostLayerMenu: boolean = false;
  @observable parentFeatureId:number = 0;
  @observable currentFeatureId:number = 0;

  // お知らせ
  @observable showInformationMenu: boolean = false;

  // 投稿管理画面 親featureId
  @observable postSttingsCurrentParentId:number = 0;
  // 投稿管理画面 属性パネル　呼び出しイベント
  @observable postSttingsAttributePanaleAfterEvent:any = null;
  // テーマId
  @observable selectedThemeId:string = "";
  // テーマId(一時保持用)
  @observable selectedTempThemeId:string = "";
  // テーマグループ名
  @observable selectedThemeGroupName:string = "";
  // テーマ名
  @observable selectedThemeName:string = "";
  // テーマ内切替項目値
  @observable selectedWithThemeSwitchItemValue:string = "";
  // フルスクリーンモード
  @observable fullScreenMode:number = 0;
  // 連携対象のfeatureIdリスト
  @observable linkedFeatureIdList:any = {};
  // 連携対象のリスト検索値
  @observable linkedListSearchData:any = {};
  // テーマレイアウト
  @observable displayLayout:any = [];
  // 更新対象用レイアウト
  @observable updateLayout:any = [];
  // テーマ情報
  @observable themeData:any = {};
  // テーマ一覧情報
  @observable themeDataList:any=[];
  // 表示対象のグラフリストデータ
  @observable displayGraphListData:any = {};
  // グラフリストデータ内部保持用
  @observable graphList:any = {};
  // 管理画面ページタイトル
  @observable adminPageTitle:string = "";
  // 管理画面ページNo
  @observable adminPageNum:number = 0;
  // レイヤ編集画面
  @observable showLayer: boolean = false;
  // 初期表示フラグ
  @observable initShowFlag: boolean = true;
  ...
  @action
  showLoginPanel() {
    this.terria.analytics?.logEvent(Category.login, CustomAction.panelOpened);
    this.showLoginMenu = true;
    this.setTopElement("LoginPanel");
  }
  ...
  @action
  showSearchPanel(){
    this.terria.analytics?.logEvent(Category.search, CustomAction.panelOpened);
    this.showSearchMenu = true;
    this.setTopElement("SearchPanel");
  }
  ...
  @action
  showActivityPanel(){
    this.terria.analytics?.logEvent(Category.activity, CustomAction.panelOpened);
    this.showActivityMenu = true;
    this.setTopElement("ActivityPanel");
    this.terria.showActivityPanel();
  }
  
  //活動登録パネルの表示(編集モード)
  @action
  showActivityPanelById(id:number){
    this.terria.setActivityId(id);
    this.terria.analytics?.logEvent(Category.activity, CustomAction.panelOpened);
    this.showActivityMenu = true;
    this.setTopElement("ActivityPanel");
    this.terria.showActivityPanel();
  ...
  showPostLayerPanel(){
    ...
    this.terria.showPostLayerPanel();
  }

  //投稿レイヤパネルの表示
  @action
  showPostLayerPanelById(id:number){
    this.terria.setFeatureId(id);
    this.terria.analytics?.logEvent(Category.activity, CustomAction.panelOpened);
    this.showPostLayerMenu = true;
    this.terria.showPostLayerPanel();
  }
  ...
  hidePostLayerPanel() {
    ...
    this.terria.hidePostLayerPanel();
    this.setTopElement("");
  }
  ...
  showInformationPanel(){
    this.terria.analytics?.logEvent(Category.information, CustomAction.panelOpened);
    this.showInformationMenu = true;
    this.setTopElement("InformationPanel");
  }

  //お知らせパネルの非表示
  @action
  hideInformationPanel(){
    this.showInformationMenu = false;
  }

  //レイヤ編集パネルの表示
  @action
  showLayerPanel(layer:any){
    this.terria.analytics?.logEvent(Category.information, CustomAction.panelOpened);
    this.layer = layer;
    this.showLayer = true;
    this.setTopElement("LayerPanel");
  }
  ...
  hideAllPanel(){
    ...
    try{
      ...
      for (const aItem of items) {
        if ((aItem.uniqueId === 'エリマネ活動登録地点') || aItem.uniqueId === '開始地点' || aItem.uniqueId === '終了地点' || aItem.uniqueId === 'レイヤ投稿地点' || aItem.uniqueId === '経路探索' || aItem.uniqueId === '//データセット/共通/歩行者ネットワークデータ/歩行者空間ネットワーク(2D表示)') {
  ...
  @action
  changeActivityMode(){
    const themeLayerList:any = [...this.terria.themeLayerList];
    let activityPostTypeIndex = themeLayerList.findIndex((themeLayer:any)=>themeLayer.layerType == 2 && themeLayer.postFlag == 1);
    let normalPostTypeIndex = themeLayerList.findIndex((themeLayer:any)=>themeLayer.layerType == 1 && themeLayer.postFlag == 1);
    if(activityPostTypeIndex > -1){
      this.activityType = 1;
      for(let i=0;i<themeLayerList.length;i++){
        if(themeLayerList[i].uniqueId == Config.layerUniqueIdCorrespondenceTable.event){
          this.activityType = 2;
        }
      }
      if(this.terria.mode == 1){
        this.hideActivityPanel();
      }else{
        this.terria.setMode(1);
        this.setTopElement("ActivityPanel");
        if(this.terria.useSmallScreenInterface == false){
          this.terria.hideActivityPanel();
          this.terria.showActivityPanel();
        }
      }
    }else if(normalPostTypeIndex > -1){
      if(this.terria.mode == 4){
        this.hidePostLayerPanel();
      }else{
        this.terria.setMode(4);
        this.setTopElement("PostLayerPanel");
        if(this.terria.useSmallScreenInterface == false){
          this.terria.hidePostLayerPanel();
          this.terria.showPostLayerPanel();
        }
  ...
  //活動IDをセット
  @action
  setCurrentActivityId(currentActivityId:number){
    this.currentActivityId = currentActivityId;
  }

  //親活動IDをセット
  @action
  setParentActivityId(parentActivityId:number){
    this.parentActivityId = parentActivityId;
  }

  //投稿管理画面 親活動IDまたは親featureIdをセット
  @action
  setPostSttingsCurrentParentId(postSttingsCurrentParentId:number){
    this.postSttingsCurrentParentId = postSttingsCurrentParentId;
  }

  //投稿管理画面 属性パネル　呼び出しイベントをセット
  @action
  setPostSttingsAttributePanaleAfterEvent(postSttingsAttributePanaleAfterEvent:any){
    this.postSttingsAttributePanaleAfterEvent = postSttingsAttributePanaleAfterEvent;
  }

  //フィーチャIDをセット
  @action
  setCurrentFeatureId(currentFeatureId:number){
    this.currentFeatureId = currentFeatureId;
  }

  //親フィーチャIDをセット
  @action
  setParentFeatureId(parentFeatureId:number){
    this.parentFeatureId = parentFeatureId;
  }

  //linkedFeatureIdListに連携対象の属性をセット
  @action
  setLinkedFeatureAttribute(attributeName:string){
    if(!this.linkedFeatureIdList[attributeName]){
      this.linkedFeatureIdList[attributeName] = [];
    }
  }
  
  //linkedFeatureIdListに指定の連携対象の属性がある場合featureIDをセット
  @action
  setLinkedFeatureId(attributeName:string,featureId:string){
    if(this.linkedFeatureIdList[attributeName]){ 
      const index = this.linkedFeatureIdList[attributeName].findIndex((id:any)=> featureId == id);
      if(index == -1){
        this.linkedFeatureIdList[attributeName].push(featureId);
      }
    }
  }

  //linkedFeatureIdListのfeatureIdのみを初期化
  @action
  clearLinkedFeatureId(){
    Object.keys(this.linkedFeatureIdList).forEach(key =>{
      this.linkedFeatureIdList[key] = [];
    })
  }

  //linkedFeatureIdListを初期化
  @action
  clearLinkedFeatureIdList(){
    this.linkedFeatureIdList = {};
  }

  //リスト検索値をセット
  @action
  setLinkedListSearchData(linkedListSearchData:any){
    this.linkedListSearchData = linkedListSearchData;
  }

  //レイアウトをセット
  @action
  setDisplayLayout(displayLayout:any){
    this.displayLayout = displayLayout;
  }

  //更新対象用レイアウトをセット
  @action
  setUpdateLayout(updateLayout:any){
    if((this.updateLayout == null || this.updateLayout.length<1) && updateLayout != null){
      this.updateLayout = updateLayout;
    }else if(updateLayout != null){
      for(let i=0;i<updateLayout.length;i++){
        const index = this.updateLayout.findIndex((layout:any) =>layout.i == updateLayout[i].i);
        if(index > -1){
          this.updateLayout[index] = updateLayout[i];
        }else{
          this.updateLayout.push(updateLayout[i]);
        }
      }
    }
  }
  ...
  setThemeData(themeData:any){
    ...
    this.selectedThemeGroupName = themeData.themeGroupName;
    this.selectedTempThemeId = themeData.themeId;
    this.selectedThemeId = themeData.themeId;
    this.selectedThemeName = themeData.themeName;
    if(themeData.switchItemList && Object.keys(themeData.switchItemList).length > 0){
      this.selectedWithThemeSwitchItemValue = themeData.switchItemList[0][themeData.switchItemValueColumnName];
    }
    this.linkedListSearchData = {};
    this.terria.setSelectedThemeId(this.selectedThemeId);
    this.terria.setSelectedTempThemeId(this.selectedTempThemeId);
    this.terria.setThemeName(this.selectedThemeName);
    this.terria.setThemeSwitchingAlertCalledFlag(false);
  }

  //テーマ内切替項目値をセット
  @action
  setSelectedWithThemeSwitchItemValue(selectedWithThemeSwitchItemValue:any){
    this.selectedWithThemeSwitchItemValue = selectedWithThemeSwitchItemValue;
  }

  //テーマ一覧情報をセット
  @action
  setThemeDataList(themeDataList:any){
    this.themeDataList = themeDataList;
  }

  //グラフリストデータをセット
  @action
  setDisplayGraphListData(displayGraphListData:any){
    this.displayGraphListData = displayGraphListData;
  }

  //グラフリストをセット
  @action
  setGraphList(graphList:any){
  ...
  setSelectedThemeId(selectedThemeId:string){
    ...
    this.terria.setSelectedThemeId(selectedThemeId);
  }

  //選択されたthemeグループ名をセット
  @action
  setThemeGroupName(selectedThemeGroupName:string){
    this.selectedThemeGroupName = selectedThemeGroupName;
  }

  //選択されたthemeIdをセット(一時保持用)
  @action
  setSelectedTempThemeId(selectedTempThemeId:string){
    this.selectedTempThemeId = selectedTempThemeId;
    this.terria.setSelectedTempThemeId(selectedTempThemeId);
  }

  //フルスクリーンモードをセット
  @action
  setFullScreenMode(fullScreenMode:number){
    this.fullScreenMode = fullScreenMode;
  }

  //管理画面のページNo及びページタイトルをセット
  @action
  setAdminPageNum(adminPageNum:number){
    this.adminPageNum = adminPageNum;
    switch(adminPageNum){
      case 0:
        this.adminPageTitle = "管理者機能用ログイン画面";
      case 1:
        this.adminPageTitle = "テーマ・レイヤ公開管理画面";
        break;
      case 2:
        this.adminPageTitle = "ダッシュボード管理画面";
        break;
      case 3:
        this.adminPageTitle = "活動・投稿情報管理画面";
        break;
      case 4:
        this.adminPageTitle = "地域統計・回遊性情報管理画面";
        break;
      case 5:
        this.adminPageTitle = "ユーザ管理画面";
        break;
    }
  }
  ...
  //管理画面のページタイトルをセット
  @action
  setAdminPageTitle(adminPageTitle:string){
    this.adminPageTitle = adminPageTitle;
  }

  //初期表示フラグをセット
  @action
  setInitShowFlag(initShowFlag:boolean){
    this.initShowFlag = initShowFlag;
  }
