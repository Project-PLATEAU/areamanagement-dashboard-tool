...
export default class Workbench {
  ...
  remove(item: BaseModel) {
    ...
    //item.terria.removeItemCookie(item);
  }
  ...
  @action
  private customInsertItem(item: BaseModel, index: number = 0) {
    if (this.contains(item)) {
      return;
    }
  ...
  public async add(item: BaseModel | BaseModel[]): Promise<Result<unknown>> {    
    ...
    //item.terria.addItemCookie(item);
  ...
  public async customAdd(item: BaseModel | BaseModel[]): Promise<Result<unknown>> {    
    if (Array.isArray(item)) {
      const results = await Promise.all(item.reverse().map(i => this.add(i)));
      return Result.combine(results, {
        title: i18next.t("workbench.addItemErrorTitle"),
   ...
   */
  @action
  customAddItemToIndex(item: BaseModel, newIndex: number) {
    this.remove(item);
    const referenceItem = item.sourceReference ? item.sourceReference : item;
    this._items.splice(newIndex, 1, referenceItem);
  }
}
