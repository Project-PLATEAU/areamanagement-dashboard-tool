...
export default class Terria {
  ...
  readonly configParameters: Complete<ConfigParameters> = {
    ...
    serverConfigUrl: "serverconfig.json",
    shareUrl: "share",
  ...
  /**
   * modelUniqueIdのcookieデータを初期化
   */
  @action
  initItemCookie(){
    let date1: Date = new Date();
    let dayOfExpire = 1 //1日
    date1.setTime(date1.getTime() + dayOfExpire * 24 * 60 * 60 * 1000);
    let date2 = date1.toUTCString();
    let uri = new URL(window.location.href);
    let domain = uri.hostname
    let path = uri.pathname
    document.cookie = "modelUniqueId=;expire="+date2+";domain="+domain+";path="+path
  }

  /**
   * modelUniqueIdのcookieデータに追加
   * @param {any} item
   */
  @action
  addItemCookie(item: any){
    //addしたitemのuniqueIdがCookieに無い時追加
    let cookie = document.cookie
    let cookiesArray = cookie.split(';');
    let cookieModels: string[] = []
    for (let c of cookiesArray) {
      let cArray = c.split('=');
      if (cArray[0].trim() == 'modelUniqueId' && cArray[1].length > 0) {
        cookieModels = cArray[1].split(',')
      }
    }
    ...
    let domain = uri.hostname
    let path = uri.pathname
    if (uniqueId != undefined && !cookieModels.includes(uniqueId)) {
      if(uniqueId !== "/" && uniqueId !== "___$FeatureHighlight&__"){
        cookieModels.push(uniqueId)
        document.cookie = 'modelUniqueId=' + cookieModels + ";expire="+date2+";domain="+domain+";path="+path
      }
    }
  }

  /**
   * modelUniqueIdのcookieデータから削除
   * @param {any} item
   */
   @action
   removeItemCookie(item: any){
     //removeしたitemのuniqueIdがCookieにある時削除
    let cookie = document.cookie
    let cookiesArray = cookie.split(';');
    let cookieModels: string[] = []
    for (let c of cookiesArray) {
    ...
    if (uniqueId != undefined && cookieModels.includes(uniqueId)) {
      cookieModels.splice(cookieModels.indexOf(uniqueId), 1)
      document.cookie = 'modelUniqueId=' + cookieModels + ";expire="+date2+";domain="+domain+";path="+path
    }
   }
  ...
  async start(options: StartOptions) {
    ...
    if (options.applicationUrl) {
      ...
      //ウインドウ（タブ）を閉じた時、cookieを消去する
      //更新した時でも削除される
      let domain = options.applicationUrl.hostname
      let path = options.applicationUrl.pathname
      ...
      window.addEventListener('unload', () => {
        ...
        document.cookie = 'modelUniqueId=;max-age=0;domain=' + domain + ";path=" + path
      })
    }
  ...
  async addCookieData() {
    ...
    for (let i = 0; i < cookieModels.length; i++) {
      let existingReference = this.getModelById(BaseModel, cookieModels[i]);
      if (existingReference === undefined || this.workbench.contains(existingReference) || existingReference.TraitsClass.name == "CatalogGroupTraits" || existingReference.TraitsClass.name == "GeoJsonCatalogItemTraits") {
        continue
      } else {
        if (!this.workbench.itemIds.includes(cookieModels[i])) {
          this.workbench.add(existingReference)
        }
      }
    }
  }
  ...
  async applyInitData({
    ...
    if(activateStory){
      this.initItemCookie();
      for (const aItem of newItems) {
        this.addItemCookie(aItem);
      }
    }
        ...
        runInAction(() => {
          terria.initSources.push(
            ...startDataV8!.initSources.map((initSource: any) => {
              ...
              //queryの値とcookieの値とを比較し、cookieに無いものを追加する
              let cookie = document.cookie
              //cookieの期限・ドメイン・パスを作成
              let date1: Date = new Date();
              let dayOfExpire = 1 //1日
              date1.setTime(date1.getTime() + dayOfExpire * 24 * 60 * 60 * 1000);
              let date2 = date1.toUTCString();
              ...
              //QueryのmodelsとCookieのmodelsとを比較
              let cookieModels: string[] = []
              var cookiesArray = cookie.split(';');
              for (var c of cookiesArray) {
                var cArray = c.split('=');
              ...
              let queryModels = initSource.models

              //Cookieに無いデータの場合追加
              let queryModelsKeys = Object.keys(queryModels)
              var cnt = 0
              for (let i = 0; i < queryModelsKeys.length; i++) {
                if (cookieModels.includes(queryModelsKeys[i])) {
                  cnt++;
                } else {
                  if(queryModelsKeys[i] !== "/" && queryModelsKeys[i] !== "___$FeatureHighlight&__"){
                    cookieModels.push(queryModelsKeys[i])
                  }
                }
              }
              //cookieを設定
              document.cookie = 'modelUniqueId=' + cookieModels + ";expires=" + date2 + ";domain=" + domain + ";path=" + path
              return {
                name,
                data: initSource,
                errorSeverity
