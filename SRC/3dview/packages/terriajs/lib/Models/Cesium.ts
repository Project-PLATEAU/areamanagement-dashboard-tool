...
export default class Cesium extends GlobeOrMap {
    ...
    inputHandler.setInputAction(e => {
      ...
      if (pickPositionCartographic) {
        ...
        //mode=1の時は活動登録
          ...
          this.terria.setEventId(0);
          if (!this.isFeaturePickingPaused){
            this.pickFromScreenPosition(e.position, false);
          }

        //mode=2の時は経路探索の開始地点を追加
        ...
        //mode=3の時は経路探索の終了地点を追加
          ...
          const items = this.terria.workbench.items;
          for (const aItem of items) {
              if (aItem.uniqueId === '終了地点') {
                  this.terria.workbench.remove(aItem);
              }
          }
          const pointItem = new GeoJsonCatalogItem("終了地点", this.terria);
          pointItem.setTrait(CommonStrata.user, "geoJsonData", {
            type: "Feature",
            properties: {
              "marker-color": "#dc143c",
   ...
   *
   */
  pickFromScreenPosition(screenPosition: Cartesian2, ignoreSplitter: boolean,hashUrlTransitionId?: string) {
    const pickRay = this.scene.camera.getPickRay(screenPosition);
    const pickPosition = this.scene.globe.pick(pickRay, this.scene);
    const pickPositionCartographic =
      pickPosition && Ellipsoid.WGS84.cartesianToCartographic(pickPosition);

    const vectorFeatures = this.pickVectorFeatures(screenPosition,hashUrlTransitionId);
   ...
   */
  pickVectorFeatures(screenPosition: Cartesian2,hashUrlTransitionId?:string) {
    // Pick vector features
    const vectorFeatures = [];
    let pickedList = this.scene.drillPick(screenPosition);
    if(hashUrlTransitionId && isDefined(hashUrlTransitionId) && hashUrlTransitionId !==null && hashUrlTransitionId !== ""){
      pickedList = this.scene.drillPick(screenPosition,100,500,500);
    }
    ...
    result.allFeaturesAvailablePromise = Promise.all(featurePromises)
      .then(allFeatures => {
        runInAction(() => {
          ...
          result.features = allFeatures.reduce(
            (resultFeaturesSoFar, imageryLayerFeatures, i) => {
              ...
              let features:any = imageryLayerFeatures.map(feature => {
                ...
                if(feature?.data?.properties?.event_id){
                  feature.description = feature.description?.replace(/\n/g,'<br>');
                  this.terria.setEventId(feature.data.properties.event_id);
                }
          ...
          if(hashUrlTransitionId && isDefined(hashUrlTransitionId) && hashUrlTransitionId !== null && hashUrlTransitionId !== ""){
            let newFeatures:any[] = [];
            let spot = Config.feature.featurePropertyNameForExcursionAnalysisId.spot;
            let excursion = Config.feature.featurePropertyNameForExcursionAnalysisId.excursion;
            let activityId = Config.feature.featurePropertyNameForParentActivityId;
            for(let i = 0; i< result.features.length;i++){
              if(result.features[i] && result.features[i].properties){
                ...
                if(tempPropertieNames && tempProperties && (tempPropertieNames.includes(spot) || tempPropertieNames.includes(excursion) || tempPropertieNames.includes(activityId))){
                    let matchFlag = false;
                    Object.keys(tempProperties).forEach(key => {
                      if(tempProperties && tempProperties[key].toString() === hashUrlTransitionId){
                        matchFlag =true;
                      }
                    })
                    if(matchFlag){
        ...
        if(this.terria.mode === 1 && this.terria.activityId < 1 && this.terria.eventId < 1 && this.terria.permission === "admin"){
          let res = false;
          res = window.confirm("選択した地点に活動結果を追加しますか？");
          if (res) {
            const activityPanelControll = document.getElementById('activityPanelControll') as HTMLElement;
            activityPanelControll.click();
          }
        }
      })
      .catch(() => {
