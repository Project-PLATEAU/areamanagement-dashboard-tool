...
import { item } from "../ReactViews/Custom/Chart/tooltip.scss";
...
export default class Cesium extends GlobeOrMap {
  ...
  constructor(terriaViewer: TerriaViewer, container: string | HTMLElement) {
    ...
    inputHandler.setInputAction(e => {
      ...
      //ソートを行う(もしエリマネ・イベント投稿と投稿レイヤが同時に紐づく場合はエリマネ・イベント投稿を優先に)
      themeLayerList.sort((a,b)=>a["layerType"]-b["layerType"]);
      //layerType=1 or layerType=2のテーマのレイヤ一覧のindexを取得
      const index = themeLayerList.findIndex((themeLayer:any)=>(themeLayer["layerType"]==1 || themeLayer["layerType"]==2) && themeLayer.postFlag == 1);
      let iconPath = Config.config.plateauPath + "/sample/sample_b_3.png";
      if(index > -1){
          //アイコン画像のパスを取得
          if(themeLayerList[index]["iconPath"]){
            iconPath = themeLayerList[index]["iconPath"];
          }
      }
      if (pickPositionCartographic) {
        ...
        if (this.terria.mode === 0) {
          ...
          for (const aItem of items) {
            if ((aItem.uniqueId === 'エリマネ活動登録地点') || aItem.uniqueId === '出発地点' || aItem.uniqueId === '到着地点' || aItem.uniqueId === 'レイヤ投稿地点') {
              this.terria.workbench.remove(aItem);
            }
          }

        //mode=1の時は活動登録
        }else if (this.terria.mode === 1) {
          const selectedTempThemeId = this.terria.selectedTempThemeId;
          const selectedThemeId = this.terria.selectedThemeId;
          const themeName = this.terria.selectedThemeName;
          if(selectedTempThemeId != selectedThemeId && !this.terria.themeSwitchingAlertCalledFlag){
            if(!window.confirm("現在は"+themeName+"がテーマ表示されています。登録を開始してよろしいですか？")){
              return;
            }else{
              this.terria.setThemeSwitchingAlertCalledFlag(true);
            }
          }
          if(this.terria.useSmallScreenInterface == true){
            this.terria.hideActivityPanel();
          }
          this.terria.setClickLatLong(latitude + "," + longitude);
          this.terria.setActivityId(0);
          const items = this.terria.workbench.items;
          for (const aItem of items) {
          ...
          const pointItem = new CzmlCatalogItem("エリマネ活動登録地点", this.terria);
          let czmlObj = [
            {
              id: "document",
              name: "line",
          ...
          pointItem.setTrait(CommonStrata.definition, "czmlString", JSON.stringify(czmlObj));
          ...
          if(isDefined(pickPosition)){
            this._selectionIndicator.position = pickPosition;
        ...
        //mode=2の時は経路探索の出発地点を追加
          ...
          const items = this.terria.workbench.items;
          for (const aItem of items) {
              if (aItem.uniqueId === '出発地点' || aItem.uniqueId === '到着地点') {
                  this.terria.workbench.remove(aItem);
              }
          }
          const pointItem = new GeoJsonCatalogItem("出発地点" , this.terria);
        ...
        //mode=3の時は経路探索の到着地点を追加
        } else if (this.terria.mode === 3) {
          this.terria.setRouteEnd(latitude + "," + longitude);
          this.terria.setMode(0);
          const routeEnd = document.getElementById('routeEnd') as HTMLInputElement;
          routeEnd.value = this.terria.routeEnd;
          const items = this.terria.workbench.items;
          for (const aItem of items) {
              if (aItem.uniqueId === '到着地点') {
                  this.terria.workbench.remove(aItem);
              }
          }
          const pointItem = new GeoJsonCatalogItem("到着地点", this.terria);
          pointItem.setTrait(CommonStrata.user, "geoJsonData", {
            type: "Feature",
            properties: {
              "marker-color": "#dc143c",
        ...
        //mode=4の時はレイヤ投稿
          ...
          const pointItem = new CzmlCatalogItem("レイヤ投稿地点", this.terria);
          let czmlObj = [
            {
              id: "document",
              name: "line",
   ...
   *
   */
  pickFromScreenPosition(screenPosition: Cartesian2, ignoreSplitter: boolean,hashUrlTransitionIdList?: string[],keyNameList?: string[]) {
    const pickRay = this.scene.camera.getPickRay(screenPosition);
    const pickPosition = this.scene.globe.pick(pickRay, this.scene);
    const pickPositionCartographic =
      pickPosition && Ellipsoid.WGS84.cartesianToCartographic(pickPosition);

    const vectorFeatures = this.pickVectorFeatures(screenPosition,hashUrlTransitionIdList);
    ...
    let imageryLayers: ImageryLayer[] = [];
    let result: any = null;
    if(isDefined(hashUrlTransitionIdList) && hashUrlTransitionIdList != null && isDefined(keyNameList) && keyNameList != null){
      if (this.terria.allowFeatureInfoRequests && pickPositionCartographic != undefined) {
        for (let i = this.scene.imageryLayers.length - 1; i >= 0; i--) {
          const imageryLayer = this.scene.imageryLayers.get(i);
      ...
      result = this._buildPickedFeatures(
        providerCoords,
        pickPosition,
        vectorFeatures,
   ...
   */
  pickVectorFeatures(screenPosition: Cartesian2,hashUrlTransitionIdList?:string[]) {
    // Pick vector features
    const vectorFeatures = [];
    let pickedList = this.scene.drillPick(screenPosition);
    if(isDefined(hashUrlTransitionIdList) && hashUrlTransitionIdList !==null){
      pickedList = this.scene.drillPick(screenPosition,100,300,500);
    }else{
      pickedList = this.scene.drillPick(screenPosition,20,50,50);
    }
  ...
  private _buildPickedFeatures(
    ...
    hashUrlTransitionIdList?:string[],
    keyNameList?:string[]
  ): PickedFeatures {
        ...
        runInAction(() => {
          ...
          result.features = allFeatures.reduce(
            (resultFeaturesSoFar, imageryLayerFeatures, i) => {
              ...
              let features:any = imageryLayerFeatures.map(feature => {
                ...
                if(feature?.data?.properties?.activity_id){
                  feature.description = feature.description?.replace(/\n/g,'<br>');
                  this.terria.setActivityId(feature.data.properties.activity_id);
                }
                if(feature?.data?.properties?.feature_id){
                  feature.description = feature.description?.replace(/\n/g,'<br>');
                  this.terria.setFeatureId(feature.data.properties.feature_id);
                }
              ...
              if (
                ...
                features = features.filter(({feature}: {feature: any}) => {
          ...
          if(isDefined(hashUrlTransitionIdList) && hashUrlTransitionIdList != null && isDefined(keyNameList) && keyNameList != null){
            let newFeatures:any[] = [];
            let featureIndexList:number[] = [];
            for(let i = 0; i< result.features.length;i++){
              for(let j = 0; j< hashUrlTransitionIdList.length;j++){
                let hashUrlTransitionId = hashUrlTransitionIdList[j];
                let keyName = "";
                if(keyNameList[j]){
                  keyName = keyNameList[j];
                }
                if(result.features[i] && result.features[i].properties){
                  let tempPropertieNames = result.features[i].properties?._propertyNames;
                  let tempProperties = result.features[i].properties;
                  if(tempPropertieNames && tempProperties && (tempPropertieNames.includes(keyName))){
                      let matchFlag = false;
                      Object.keys(tempProperties).forEach(key => {
                        if(tempProperties && tempProperties[key]){
                          let featurePropertieValue:string = tempProperties[key].toString();
                          try{
                            featurePropertieValue = featurePropertieValue.trim();
                            hashUrlTransitionId = hashUrlTransitionId?.trim();
                          }catch(e){}
                          if(tempProperties && featurePropertieValue === hashUrlTransitionId){
                            matchFlag =true;
                          }
                        }
                      })
                      if(matchFlag && featureIndexList.findIndex(featureIndex=>featureIndex==i) < 0){
                        featureIndexList.push(i);
                        newFeatures.push(result.features[i]); 
                      }
                  }
                }
              }
          ...
          try{
            //CZML等で複数ある同一のfeatureは重複排除
            result.features = Array.from(
              new Map(result.features.map((feature:any) => [feature.id, feature])).values()
            );
            //属性表示非表示のlayerは対象外とする
            if(hashUrlTransitionIdList == undefined || hashUrlTransitionIdList == null || Object.keys(hashUrlTransitionIdList).length < 1){
              result.features = result.features.filter((feature: any) => {
                return (
                  Config.attributeDisplayExclusionListForLayerName.findIndex(layerName=>(feature?.imageryLayer?.imageryProvider?._layers == layerName || feature?._catalogItem?.uniqueId == layerName)) < 0
                );
              });
            }
          }catch(e){}
