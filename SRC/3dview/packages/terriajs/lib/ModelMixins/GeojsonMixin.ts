      ...
      for (let i = 0; i < geoJson.features.length; i++) {
        ...
        if (feature !== null && feature.geometry?.type === "Point") {
          ...
          if (feature.properties !== null) {
            if(czml.billboard && czml.billboard.image && czml.billboard.customImage && czml.billboard.customImage.startsWith("#properties.")){
              if(czml.billboard.customImage.startsWith("#properties.feature_id") && feature.properties.layer_id){
                //feature_idの指定の場合は画像APIから取得（オリジナル仕様）
                let postLayerSelectOption = Config.postLayerSelectOption;
                let itemId = 0;
                for(let i=0;i<postLayerSelectOption.length;i++){
                  const target:any = postLayerSelectOption[i];
                  if(target?.layerId ==feature.properties.layer_id ){
                    itemId = target?.itemId;
                    break;
                  }
                }
                
                let param = "?featureId=" + feature.properties.feature_id + "&itemId=" + itemId;
                czml.billboard.image = Config.config.billboardUrlForPostLayer + param;
              }else if(czml.billboard.customImage.startsWith("#properties.participants_count") && feature.properties.participants_count != undefined){
                //participants_count(参加者数)の指定の場合は画像APIから取得（オリジナル仕様）
                czml.billboard.image = Config.config.billboardUrlForActivity + "?participantsCount=" + feature.properties.participants_count;
              }else{
                //プロパティ値をパスにセット（汎用仕様）
                czml.billboard.image = feature.properties[czml.billboard.customImage.replaceAll("#properties.","")];
              }
            }
...
function describeWithoutUnderscores(
  ...
  for (let key in properties) {
    if (properties.hasOwnProperty(key)) {
      ...
      if(properties["searchMode"] === "on" && key === "_id_"){
        continue;
      }
  ...
  if (html.length > 0) {
    html =
      '<table style="width:100%;" class="cesium-infoBox-defaultTable"><tbody>' +
      html +
      "</tbody></table>";
  }
  return html;
