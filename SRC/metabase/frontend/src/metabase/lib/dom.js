...
export function is3dViewUrl(url) {
  const plateauUrl = MetabaseSettings.getPlateauUrl();
  a.href = url;
  const url3dView = a.origin + plateauUrl;
  return url.startsWith(url3dView);
}

export function isImageViewUrl(url) {
  const apiUrl = MetabaseSettings.getApiUrl();
  a.href = url;
  const url3dView = a.origin + apiUrl;
  return url.startsWith(url3dView);
}

export function getUrlTarget(url) {
  return isSameOrigin(url) ? (is3dViewUrl(url) ? "_3dview" : (isImageViewUrl(url) ? "_attachement" : "_self")) : "_blank";
}
