	...
	public List<RouteSearch> searchRouteWithCost(int startNodeId, int endNodeId, int condition, int resSRID) {
		...
		try {
			...
			String sql = "" + //
					...
					"ST_XMin(ST_Transform(united.geom, :srid)) AS minlon, " + //
					"ST_XMax(ST_Transform(united.geom, :srid)) AS maxlon, " + //
					"ST_YMin(ST_Transform(united.geom, :srid)) AS minlat, " + //
					"ST_YMax(ST_Transform(united.geom, :srid)) AS maxlat, " + //
