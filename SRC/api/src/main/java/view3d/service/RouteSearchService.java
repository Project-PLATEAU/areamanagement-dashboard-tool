...
public class RouteSearchService {
	...
	/** 画面表示座標系 */
	@Value("${app.route.view.epsg}")
	private Integer viewEPSG;
	/** データ座標系 */
	@Value("${app.route.data.epsg}")
				...
				for (int j = 0; j < 2; j++) {
					...
					if (routeSearchResult.size() > 0 && routeSearchResult.get(0).getGeojson() != null) {
						...
						if (routeSearchResult.size() == 1) {
							...
							routeSearchResultForm.setMinlon(routeSearchResult.get(0).getMinlon());
							routeSearchResultForm.setMaxlon(routeSearchResult.get(0).getMaxlon());
							routeSearchResultForm.setMinlat(routeSearchResult.get(0).getMinlat());
							routeSearchResultForm.setMaxlat(routeSearchResult.get(0).getMaxlat());
							...
							// 複数結果が得られた場合、取得結果を比較
							double minDistance = routeSearchResult.get(0).getDistance();
							// 最短経路を取得
							for (int k = 1; k < routeSearchResult.size(); k++) {
								minDistance = (routeSearchResult.get(k).getDistance() < minDistance)
										? routeSearchResult.get(k).getDistance()
										: minDistance;
							}
							for (int k = 0; k < routeSearchResult.size(); k++) {
								//優先度高いものから順に最短経路と比較した距離を検査し、一定倍率以内であれば返す。
								if (routeSearchResult.get(k).getDistance() < minDistance
										* ALLOW_DISTANCE_MAGNIFICATION) {
									routeSearchResultForm.setResult(routeSearchResult.get(k).getGeojson());
									routeSearchResultForm.setMinlon(routeSearchResult.get(k).getMinlon());
									routeSearchResultForm.setMaxlon(routeSearchResult.get(k).getMaxlon());
									routeSearchResultForm.setMinlat(routeSearchResult.get(k).getMinlat());
									routeSearchResultForm.setMaxlat(routeSearchResult.get(k).getMaxlat());
									routeSearchResultForm.setDistance(routeSearchResult.get(k).getDistance());
									routeSearchResultForm.setResultPriority(routeSearchResult.get(k).getPriority());
									LOGGER.debug("検索結果をControllerに返却");
									return routeSearchResultForm;
								}
							}
