...
search_activity as (
    ...
    WHERE
        CASE
            ...
            WHEN login_user.role = 'admin' OR login_user.role = 'erimane' THEN activity.activity_type = 1
            --userの場合自身の投稿or公開フラグが1のもの
            WHEN login_user.role = 'user' THEN activity.activity_type = 1
            AND (
                activity.publish_flag = '1'
                OR activity.post_user_id = login_user.user_id
            ) 
            --それ以外の未ログインユーザは公開フラグが1のもの
            ELSE activity.activity_type = 1 AND activity.publish_flag = '1'
        END
    group by
        parent_activity_id,
        activity_type
...
select
    ...
    END as height,
    CASE
        WHEN ST_Z(ST_TransForm(activity.geom, 4326)) IS NULL THEN ST_Force3D(activity.geom, 250)
        ELSE geom
    END as geom,
...
from
    ...
    FULL OUTER JOIN login_user ON 1 = 1
WHERE
    CASE
        --管理者 or erimaneは閲覧制限なし
        WHEN login_user.role = 'admin' OR login_user.role = 'erimane' THEN activity.activity_type = 1 
